...
sub read_bib_entry
  {
	...
	if (/\G\s*([\w\-\.\:]+)\s*=\s*([^,\"\{\}\(\)\s]+)\s*,?/cg) {
	  ...
	  $wholeentry .= ",\n  " . $1 . ' = "' . $be{lc($1)} .'"' 
	    if (!($1 =~ m/$conf{'remove_fields_regexp'}/i));
	  next
	}
	...
	if (/\G\s*([\w\-\.\:]+)\s*=\s*\"(.*?[^\\])\"\s*,?/cg) {
	  ...
	  $be{lc($field)} =~ s/\ +/ /g;
	  $wholeentry .= ",\n  " . $field . ' = "' . $be{lc($field)} .'"'
	    if (!($field =~ m/$conf{'remove_fields_regexp'}/i));
	  next
	}
	...
	return -1;
      ...
      $be{'ALTKEYS'} = [];
      push @bibentry, \%be if (!(exists $be{'bib2htmlskip'}));
...
sub htmlfm_entry_basic
  {
    my ($beref) = shift @_;
    my ($titlefirst) = shift @_;
    my ($ret);
    $ret = '';
  SWITCH: for ($beref->{"TYPE"}) {
      /article/ && do {
	$ret .= '<span class="b2hTitle">'.htmlfm_title($beref->{"title"}).'. </span>' if ($titlefirst);
	$ret .= '<span class="b2hAuthor">'.htmlfm_author($beref->{"author"}).'. </span>';
	$ret .= '<span class="b2hTitle">'.htmlfm_title($beref->{"title"}).'. </span>' if (!$titlefirst);
	$ret .= '<span class="b2hPubInfo">';
	...
	last
      ...
      /book/ && do {
	$ret .= '<span class="b2hTitle"><i>'.htmlfm_title($beref->{"title"}).'</i>, </span>' if ($titlefirst);
	if (exists $beref->{"author"}) {
	  $ret .= '<span class="b2hAuthor">'.htmlfm_author($beref->{"author"}).'. </span>';
	} else {
	  $ret .= '<span class="b2hAuthor">'.htmlfm_author($beref->{"editor"}).', editors. </span>';
	}
	$ret .= '<span class="b2hTitle"><i>'.htmlfm_title($beref->{"title"}).'</i>, </span>' if (!$titlefirst);
	$ret .= '<span class="b2hPubInfo">';
    ...
    #stuff common to all
    if (exists $beref->{'notehtml'}) {
      $ret .= ' <span class="b2hNote">'.htmlfm_from_tex($beref->{'notehtml'}). '</span>';
    } else{
      $ret .= ' <span class="b2hNote">'.htmlfm_from_tex($beref->{'note'}). '</span>' if (exists $beref->{'note'});
    }
    $ret .= ' <span class="b2hWWWNote">'.htmlfm_from_tex($beref->{$conf{'bibfield_wwwnote'}}) .'</span>' if (exists $beref->{$conf{'bibfield_wwwnote'}});
...
sub get_xml_paper_info
  {
    ...
    $output .= "<citation><![CDATA[".(htmlfm_entry_basic($beref, 0))."]]></citation>\n";
    $output .= "<citationtitlefirst><![CDATA[".(htmlfm_entry_basic($beref, 1))."]]></citationtitlefirst>\n";
    $output .= get_xml_download_links($beref);
    $output .= "<abstract><![CDATA[<p>".htmlfm_from_tex($beref->{$conf{'bibfield_abstract'}})."</p>]]></abstract>\n" if ($long && exists($beref->{$conf{'bibfield_abstract'}}));
    ...
    $output .= "<errata><![CDATA[".$beref->{$conf{'bibfield_errata'}}."]]></errata>\n" if (exists($beref->{$conf{'bibfield_errata'}}));
    $output .= "</paper_info>\n";
    return $output;
  }
...
@file_formats = ('pdf', 'ps.gz', 'ps', 'html', 'supplemental.pdf');
